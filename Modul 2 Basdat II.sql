...
BEGIN
    DECLARE str VARCHAR(50);
    SET str = (SELECT CONCAT(
        LEFT((SELECT Nama_Pelanggan FROM Pelanggan WHERE ID_Pelanggan = p_ID_Pelanggan AND ID_Pelanggan = (SELECT ID_Pelanggan FROM Transaksi WHERE ID_Transaksi = p_ID_Transaksi)), 3),
        p_ID_Transaksi,
        'and',
        LEFT((SELECT Nama_Pegawai FROM Pegawai WHERE ID_Pegawai = p_ID_Pegawai), 3),
        p_ID_Pelanggan,
...
END$$
DELIMITER ;
SELECT Generate_Pembelian_Service_ID('PL004', 'P005', 'T004');
