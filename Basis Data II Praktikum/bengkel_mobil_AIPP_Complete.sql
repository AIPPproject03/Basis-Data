...
--
-- Host: 127.0.0.1
-- Waktu pembuatan: 19 Mar 2024 pada 16.05
...
('SP001', 'Ban', 100, 'Kendaraan', 150000),
...
('T001', 'PL001', '2021-01-01', 1350000),
('T002', 'PL002', '2021-01-02', 2050000),
('T003', 'PL003', '2021-01-08', 3000000),
('T004', 'PL004', '2021-01-04', 4000000),
('T005', 'PL005', '2021-01-05', 5000000),
('T006', 'PL002', '2021-01-06', 5850000);
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `daftar_pegawai`  AS SELECT `pg`.`ID_Pegawai` AS `ID_Pegawai`, `pg`.`ID_Jabatan` AS `ID_Jabatan`, `pg`.`Nama_Pegawai` AS `Nama_Pegawai`, `pg`.`Alamat` AS `Alamat`, `pg`.`Username` AS `Username`, `pg`.`Password` AS `Password`, `np`.`ID_Nomor` AS `ID_Nomor`, `np`.`No_Telp` AS `No_Telp` FROM (`pegawai` `pg` join `nomor_pegawai` `np` on(`pg`.`ID_Pegawai` = `np`.`ID_Pegawai`)) ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `pemasukan_pegawai_service`  AS SELECT `pg`.`ID_Pegawai` AS `ID_Pegawai`, `pg`.`Nama_Pegawai` AS `Nama_Pegawai`, `sv`.`Nama_Service` AS `Nama_Service`, `sv`.`Harga` AS `Harga`, count(`ps`.`ID_Service`) AS `Jumlah_Service`, `sv`.`Harga`* count(`ps`.`ID_Service`) AS `Total_Pemasukan` FROM ((`pegawai` `pg` join `pembelian_service` `ps` on(`pg`.`ID_Pegawai` = `ps`.`ID_Pegawai`)) join `service` `sv` on(`ps`.`ID_Service` = `sv`.`ID_Service`)) WHERE `pg`.`ID_Jabatan` = 'J003' GROUP BY `pg`.`ID_Pegawai`, `sv`.`ID_Service` ;
